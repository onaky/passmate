...
export default function AbsorbPage() {
  ...
  return (
    <div className="px-5">
      <PageHeader
        title="💡 AI 해설"
        subtitle="저장하면 퀴즈에서 다시 만날 수 있어요"
        backHref="/scan"
      />
      ...
      <div className="flex flex-col gap-4 pb-6">
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {result.keywords.map((kw) => (
              <span
                key={kw}
                className="px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 text-sm font-medium border border-indigo-500/30"
              >
                {kw}
              </span>
            ))}
        ...
        <div className="bg-[var(--card)] border border-amber-500/30 rounded-2xl p-5">
          ...
          <details className="cursor-pointer">
            <summary className="text-xs text-amber-400 font-semibold select-none">
              정답 보기 ▶
            </summary>
        ...
        <div className="flex flex-col gap-3 pt-2">
          {!saved ? (
            <button
              onClick={handleSave}
              disabled={saving}
              className="w-full py-4 rounded-2xl bg-indigo-500 text-white font-bold text-lg shadow-lg shadow-indigo-500/30 active:scale-95 transition-transform disabled:opacity-50"
            >
              {saving ? "저장 중..." : "📚 내 암기장에 저장하기"}
            </button>
          ) : (
            <div className="w-full py-4 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 text-center">
              <span className="text-emerald-400 font-bold">
                ✓ 저장 완료! {getMasteryLabel(0)}으로 시작해요
              </span>
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3 rounded-2xl bg-[var(--secondary)] text-[var(--foreground)] font-semibold active:scale-95 transition-transform"
            >
              📷 다시 스캔
            </button>
            <button
              ...
              className="flex-1 py-3 rounded-2xl bg-pink-500/20 border border-pink-500/30 text-pink-300 font-semibold active:scale-95 transition-transform"
            >
              🎮 퀴즈 풀기
            </button>
          </div>
